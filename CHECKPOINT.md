...
4. **WhatsApp Outreach Alternative**: Added `services/whatsapp_outreach.py` with `generate_whatsapp_link`. Added explicit `GET /api/outreach/whatsapp-links?city=Delhi` endpoint to provide a pre-canned, hyper-personalized batch list of clickable WhatsApp links targeting new leads safely prioritized by `pain_score`.
